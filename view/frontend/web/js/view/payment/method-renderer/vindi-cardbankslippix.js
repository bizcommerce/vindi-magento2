define([
    ...
    return Component.extend({
        defaults: {
            template: 'Vindi_Payment/payment/vindi-cardbankslippix',
            paymentProfiles: [],
            creditCardType: '',
            creditCardExpYear: '',
            creditCardExpMonth: '',
        ...
        initialize: function () {
            ...
            this.taxvat.value(window?.checkoutConfig?.payment?.vindi_cardbankslippix?.customer_taxvat);
        ...
        getIcons: function (type) {
            // Try to get icons from vindi_cardbankslippix config first, then fallback to vindi
            const icons = window.checkoutConfig.payment?.vindi_cardbankslippix?.icons || 
        ...
        setCardTypeFromProfile: function (profileId) {
            const savedCards = window.checkoutConfig.payment?.vindi_cardbankslippix?.saved_cards || 
        ...
        getPaymentProfiles: function () {
            let paymentProfiles = [];
            // Try to get saved cards from vindi_cardbankslippix config first, then fallback to vindi
            const savedCards = window.checkoutConfig.payment?.vindi_cardbankslippix?.saved_cards || 
                              window.checkoutConfig.payment?.vindi?.saved_cards;
